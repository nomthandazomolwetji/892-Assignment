...
1.  **\[5 marks\]** Clearly and concisely state your **research question** that you are trying to answer or explore using the steps below.

    -   *This step does not involve coding.*
...
2.  **\[5 marks\]** Clean the datasets in preparation for joining:
    ...
    -   
        ...
        d.  Ensure all variable names are in **lowercase** and contain **no special characters**, except for spaces which should be replaced with underscores (`_`).

    -   

        e.  Join the two or more datasets to create a dataframe called **`df`**.

        -   Larger datasets will get **more marks**.
...
3.  **\[5 marks\]** Create at least **two new variables** to be used in the subsequent steps.
...
4.  **\[30 marks\]** Perform a **suitable exploratory data analysis (EDA)** on the data.

    -   Must include **at least three visualisations**, but can include more.
    -   Include **accompanying text** to interpret the results.
...
**Interpretation:** This visualization reveals the relationship between trade intensity (import per capita) and unemployment outcomes. Countries with higher import intensity show varied unemployment patterns, suggesting that trade openness alone doesn't determine employment outcomes. The color coding by labor productivity categories shows that high-productivity countries can have both high and low import intensities.
...
5.  **\[15 marks\]** Calculate relevant **summary and other statistics** from the data.
...
6.  **\[15 marks\]** Create **at least two useful plots** (different from those in the EDA).
...
7.  **\[15 marks\]** Perform a **statistical or regression analysis** of the data, which could be relevant to your thesis.

    -   Marks will be awarded for **suitability and originality**.
...
**Model Performance:** - Model 2 (Enhanced) shows superior performance with higher R² and better AIC - The interaction between import intensity and agricultural value added is statistically significant - Logistic regression successfully identifies factors associated with high employment vulnerability
...
8.  **\[10 marks\]** Overall impression of your **code quality**:

    -   

        a.  Does your code run without problems?

    -   

        b.  Is your code clear and concise?

    -   

        c.  Are code blocks well described and logically structured?
...
-   The **original data** and the **accompanying code** must be **shared on OneDrive**.
-   If your code fails to run, you will be required to fix it.
-   **Late submissions:** 2 marks deducted **per day**.
